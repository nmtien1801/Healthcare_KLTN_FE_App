        ...
        >
          ...
          <View style={styles.datePickerContainer}>
            <TextInput
              ...
              value={weekStartDate}
              onChangeText={(text) =>
                handleWeekStartChange({ target: { value: text } })
              }
            ...
            <TouchableOpacity
              ...
              onPress={handleSelectCurrentWeek}
            >
          ...
          {/* Chọn ca làm việc */}
          <Text style={[styles.formLabel, styles.centered]}>
            Chọn ca làm việc
          </Text>
                ...
                <View style={styles.shiftGrid}>
                  {shiftOptions.map((shift) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.shiftButton,
                        weeklySchedule[day.key]?.includes(shift.key) &&
          ...
          {savedSchedules.length === 0 ? (
            <View style={styles.alertInfo}>
              <Text style={styles.alertText}>
                Chưa có lịch làm việc nào được lưu.
              </Text>
            </View>
          ) : (
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.scheduleItem}>
                  <Text style={styles.scheduleText}>
                    Tuần bắt đầu: {formatDate(item.weekStartDate)}
                  </Text>
                  <Text style={styles.scheduleText}>
                    Loại hình:{" "}
                    {workTypeOptions.find((wt) => wt.key === item.workType)
                      ?.label || "-"}
                  </Text>
                  <View>
                    {Object.entries(item.schedule).map(([dayKey, shifts]) => (
                      <View key={dayKey} style={styles.scheduleDay}>
                        ...
                        {shifts.length === 0 ? (
                          <Text style={styles.scheduleNoShift}>
                            Không có ca
                          </Text>
                        ) : (
          ...
          <View style={styles.doctorInfo}>
            ...
            <View style={styles.doctorInfoText}>
              <Text style={styles.doctorName}>
                {doctorInfo?.username || "Bác sĩ không xác định"}
              </Text>
        ...
        <View style={styles.scheduleDetails}>
          ...
          {currentShift && (
            <View style={styles.detailItem}>
              ...
              >{`${currentShift.start} - ${currentShift.end}`}</Text>
            </View>
          )}
        </View>
...
const AttendanceTab = () => {
      ...
      try {
        ...
        shifts.forEach((shift) => {
          ...
          const shiftKey =
            shiftOptions.find(
              (option) =>
                option.start === shift.start && option.end === shift.end
            )?.key || "parttime";
        ...
        const history = shifts
          ...
          .map((shift) => {
            ...
            if (checkInTime !== "-") {
              ...
              const [startHour, startMinute] = shift.start
                .split(":")
                .map(Number);
              ...
              if (
                inHour < startHour ||
                (inHour === startHour && inMinute <= startMinute)
              ) {
                status = "Đúng giờ";
              } else {
                status = "Đi trễ";
  ...
  useEffect(() => {
    if (showScheduleFormModal) {
      const existingSchedule = savedSchedules.find(
        (s) => s.weekStartDate === weekStartDate
      );
      if (existingSchedule) {
        setIsEditing(true);
  ...
  const handleSaveOrUpdateSchedule = async () => {
    ...
    try {
      ...
      if (isEditing) {
        const editingSchedule = savedSchedules.find(
          (s) => s.weekStartDate === weekStartDate
        );
        ...
        <View style={styles.card}>
          <View style={styles.buttonContainer}>
            ...
            <TouchableOpacity
              style={[
                styles.actionButton,
                { backgroundColor: "#17a2b8", marginLeft: 3 },
              ]}
